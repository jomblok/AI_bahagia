...
public class HelloController {
    ...
    static final int N = 12; // jumlah node
    ...
    public static List<Integer> getPath(int start, int goal, List<int[]> edges) {
        ...
        while (!pq.isEmpty()) {
            ...
            for (int[] edge : neighbours.get(now)) {
                ...
                if (minDistance[next] > distance + weight) {
                    minDistance[next] = distance + weight;
                    path[next] = now;
                    pq.add(new long[]{minDistance[next], next});
                }
            }
        }
